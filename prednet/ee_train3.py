...
N_seq_val = 40  # orig: 100 number of sequences to use for validation

# Model parameters
n_channels, im_height, im_width = (3, HEIGHT, WIDTH)
input_shape = (n_channels, im_height, im_width) if K.image_data_format() == 'channels_first' else (im_height, im_width, n_channels)
stack_sizes = (n_channels, 48, 96, 192, 384) #orig: (n_c, 48, 96, 196)
...
nt = 20 # number of timesteps used for sequences in training # orig: 10
...
# @kikyou123
errors = Reshape((-1,5))(errors) #orig 4
...
lr_schedule = lambda epoch: lr if epoch < 75 else 0.1*lr    # start with lr of 0.001 and then drop to 0.0001 after 75 epochs
...
if save_model:
    outputdir="nt"+str(nt)+"_extrap"+extrap+"_b"+str(batch_size)+"_e"+str(nb_epoch)+"_s"+str(samples_per_epoch)+"_v"+str(N_seq_val)+"_lr"+str(lr*10)
