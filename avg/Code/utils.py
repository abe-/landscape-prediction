...
def process_clip():
    ...
    for i in xrange(100):  # cap at 100 trials in case the clip has no movement anywhere
        ...
        #print clip_l2_diff(cropped_clip)
...
# AGF 31/12/2017
def get_predict_batch(test_batch_size, num_rec_out=1):
    """
    Gets a clip from the test dataset.
    ...
    @return: An array of shape:
             ...
             A batch of frame sequences with values normalized in range [-1, 1].
    """
    return get_full_clips_predict(c.TEST_DIR, test_batch_size, num_rec_out=num_rec_out)
